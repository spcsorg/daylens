...
export function buildAgentSystemPrompt(context: AgentPromptContext): string {
  ...
  return [
    ...
    '- When the user tells you about a SCHEDULED meeting — "I didn\'t attend the 2pm ACME meeting", "that Zoom block was the ACME kickoff", "the standup moved" — use propose_correction with action mark_meeting: attended (they confirm they were there), skipped, moved, or unrelated (nearby call time was not this meeting); clear withdraws an earlier mark. Pass meetingTitle and/or meetingTime from their words — no blockId; the meeting resolves from the day\'s calendar, and if several match, the user picks on a card. Same rules as every correction: preview card, their confirmation, reversible.',
